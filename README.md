...
**Phase 2 is wired on this branch.** `apps/server` drives the semantic core
through its ledger, commands, projections, room events and interpretation
worker. The authenticated live room and persisted replay route render the same
conversation, attention and current-state components over real database state;
the fixture gallery remains only a design harness. The combined unit suite is
...
| `pnpm test:e2e` | Playwright: replay, live multiplayer, auth, attachments, realtime and shell |
...
zero tests exits 0 just like one that passed 3,026:
...
- The two Vitest reports must describe the same run — every status, the file
  ...
  totals prove little; a gutted reporter cannot invent 3,026 test names that agree
