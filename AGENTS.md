...
- `pnpm lint` (= `biome check .`) — **exits 0**. Measured on this tree (699842e): 404 files checked, 16 warnings, 51 infos, **0 errors**. The warnings/infos live on the harness files (`design/*.mjs`, `scripts/mutation-ledger.mjs`); biome's overrides keep them non-fatal, and no `apps/` or `packages/` source is among them. biome prints `Diagnostics not shown: 47` — a display cap, **not** a failure; the gate is green because there are zero error-severity diagnostics. Confirm with `pnpm lint; echo $?`. Treat any future exit 1 as a real failure (per HANDOFF.md) — the usual cause is an *error-severity* diagnostic (e.g. an unformatted committed JSON/fixture) that tips the run past the diagnostic cap, not the pre-existing warnings. Note `biome check design/` targeted in isolation is a separate, redder thing (see Traps) — it is not this gate.
