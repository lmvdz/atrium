...
6. **(MUST) Exact revert → re-validate.** After a drift-induced `~` from (3), the certified span's content is restored to be byte-for-byte identical to what was signed (by revert, undo, or re-typing the same characters). The mark returns to `✓` within a render tick, with no re-certify action by the human. FAIL if it stays `~` after content again equals the signed digest, or if it re-validates while content still differs.
