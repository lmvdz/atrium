 ...
 * back in — here.
 *
 * ## Why "open at startup" is a safe predicate
 *
 ...
 * So at the moment this runs, before the socket server accepts a connection, an
 * `open` session is by definition one with no live execution. It gets the exit
 * receipt it is owed — `session_failed`, artifact `null`, an exit summary that
 * says what happened — rather than a fabricated clean one, because a run that was
 * killed did not settle and this seam does not pretend otherwise.
 *
