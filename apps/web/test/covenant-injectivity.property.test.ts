...
describe('INJECTIVITY ORACLE (property): the reader digest is injective over generated spans', () => {
  // Generate a large, deterministic corpus of compliant spans once.
  const corpus: Array<{ seed: number; spec: Spec; sig: string; outcome: string }> = [];
  for (const seed of SEEDS) {
    const rng = mulberry32(seed);
    for (let k = 0; k < 5; k++) {
      const spec = genSpec(rng);
      const outcome = readerOutcome(spec);
      corpus.push({ seed, spec, sig: specSig(spec), outcome });
    }
  }
