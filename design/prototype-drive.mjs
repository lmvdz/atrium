...
const WIDTHS = String(argOf('--widths', '1440,1366,1279,1160,1120'))
  .split(',')
  .map(Number)
  .filter(Boolean);
...
window.__occl = function () {
  ...
  for (n = w.nextNode(); n; n = w.nextNode()) {
...
async function newPage(browser, width) {
  const ctx = await browser.newContext({
    viewport: { width: Number(width) || SEQ_WIDTH, height: HEIGHT },
  });
...
async function ctlList(page) {
  return page.evaluate('window.__ctls()');
}
...
async function seqStep(page, errors, path) {
  ...
  const out = await page.evaluate(() => ({
    ...
    uncovered:
      typeof window.__uncoveredRecordWords === 'function'
        ? window
            .__uncoveredRecordWords()
            .map(
              (u) =>
                u.field +
                '  ' +
...
async function passSequences(browser, log) {
  ...
  noteCtls(root.ctls);
  root.uncovered.forEach((u) => {
    uncovered.add(u);
  });
  root.numbers.forEach((u) => {
    numbers.add(u);
  });
  root.violations.forEach((t) => {
    violations.push({ text: t, path: [], where: 'load' });
  });
  ...
  let frontier = [
    {
      ...
      path: [],
      steps: rootSteps,
      fresh: root.ctls.length,
      freshSteps: rootSteps.length,
    },
  ];
  ...
  for (let d = 1; d <= DEPTH && frontier.length; d++) {
      ...
      workers.map(async (w) => {
        for (;;) {
          ...
          if (results.length % 200 === 0)
            log(`     ... ${results.length}/${jobs.length} transitions at depth ${d}`);
        }
      }),
    );
    const nextFrontier = [];
    ...
    results.forEach(({ j, r }) => {
      r.fresh = r.ctls.filter((c) => !ctlSeen.has(c.key)).length;
    });
    results.forEach(({ j, r }) => {
      noteCtls(r.ctls);
      ...
      if (r.failed.length)
        r.failed.forEach((f) => {
          unreachable.set(f.split(' (')[0], f);
        });
      else drivenSteps.add(j.step);
      r.violations.forEach((t) => {
        violations.push({ text: t, path: j.path.slice(), where: 'sequence d' + d });
      });
    ...
    levels.push({
      depth: d,
      states: nextFrontier.length,
      expanded: expand.length,
      truncated,
      transitions: jobs.length,
      frontier: frontier.length,
      inertCtl,
      inertBoth,
    });
    log(
      `  depth ${d}: ${jobs.length} transitions driven from ${expand.length} state(s)` +
        (truncated
          ? ` (${truncated} state(s) left unexpanded — frontier budget ${BUDGET})`
          : ' (complete breadth)') +
...
async function prefill(page, key) {
  ...
  if (/id=vgo/.test(key))
    await fill('#vinput', 'checked the runbook against the staging rehearsal log');
  if (/id=rgo/.test(key))
    await fill('#rinput', 'the rehearsal log does not cover the rollback path');
...
async function passEnumerate(browser, log) {
  ...
  const note = (c) => {
    if (!seen.has(c.key))
      seen.set(c.key, { visible: false, driven: false, reason: 'never on screen' });
  ...
  const SESSIONS_PER_LAP = Number(argOf('--lap', '6')),
    STEPS = Number(argOf('--steps', '70'));
  for (;;) {
    lap++;
    ...
    for (let s = 0; s < SESSIONS_PER_LAP; s++) {
      ...
      for (let k = 0; k < STEPS; k++) {
        ...
        if (!seen.get(pick.key).driven) {
          seen.get(pick.key).driven = true;
          discoveredThisLap++;
        }
    ...
    log(
      `  lap ${lap}: ${discoveredThisLap} newly driven control(s) over ${SESSIONS_PER_LAP} loads (${seen.size} keys known)`,
    );
    if (!discoveredThisLap) {
      dry++;
      if (dry >= 2) break;
    } else dry = 0;
    if (lap >= 14) break;
...
async function passRandom(browser, log) {
  ...
  log(
    `  ${RANDOM_SESSIONS} sessions × up to ${RANDOM_CLICKS} clicks = ${clicks} clicks, across widths ${WIDTHS.join(', ')}`,
  );
...
const D1_SCAN = () => {
  ...
  const open = {};
  Object.keys(S.rooms).forEach((k) => {
    (S.rooms[k].objects || []).forEach((o) => {
  ...
  const walker = document.createTreeWalker(document.body, NodeFilter.SHOW_TEXT, {
    acceptNode(n) {
      ...
      return /\bopen\s+\d/i.test(String(n.nodeValue))
        ? NodeFilter.FILTER_ACCEPT
        : NodeFilter.FILTER_REJECT;
    },
  });
  let n;
  for (n = walker.nextNode(); n; n = walker.nextNode()) {
    ...
    if (!o) {
      out.push(
        'a surface paints ' +
          JSON.stringify('open ' + said) +
          ' and nothing on it says which record it is about',
      );
      continue;
    }
    const w = want(o);
    ...
    out.push(
      'a surface says ' +
        ...
        ' — the record says it ' +
        'was asked at ' +
        JSON.stringify(o.askedAt) +
        ' and the room clock says ' +
        JSON.stringify(S.now) +
        ', which is ' +
        JSON.stringify('open ' + (w == null ? 'a time this page cannot subtract' : w)) +
        ' · painted in ' +
        ((p.closest('[id]') || {}).id || p.className),
    );
  }
...
const ECHO_SCAN = () => {
  const words = (el, drop) => {
    ...
    c.querySelectorAll('[data-voice="human"], .mreply' + (drop || '')).forEach((n) => {
      n.remove();
    });
  ...
  document.querySelectorAll('#feed .mrow[data-obj]').forEach((el) => {
    ...
    groups[k] = groups[k] || [];
    groups[k].push({ id: el.dataset.msg, said: words(body, ', .tag'), all: words(body) });
  });
  const out = [];
    ...
    for (let i = 1; i < g.length; i++) {
      ...
      for (let n = 0; n + 1 < g[i].said.length; n++) {
        ...
        if (earlier[pair]) {
          ...
          run.push(g[i].said[n + 1]);
        } else close();
      }
      close();
      if (best.length)
        out.push(
          'two feed rows narrate ' +
            k.split('@')[0] +
            ' in the same tick and ' +
            g[i].id +
            ' repeats ' +
            JSON.stringify(best.join(' ')) +
            ' from ' +
            bestFrom +
...
const LOCATOR_SCAN = ({ words }) => {
  ...
  document.querySelectorAll('body *').forEach((el) => {
    ...
    seen[key] = 1;
    out.push(
      'a sentence this page wrote says ' +
        JSON.stringify(word) +
        ' instead of naming what it means — ' +
        JSON.stringify(raw.slice(0, 150)) +
        ' · painted in ' +
        ((el.closest('[id]') || {}).id || el.className) +
        ' at y=' +
...
const REPROS = [
  ...
  },
  {
    ...
    async run(page) {
      ...
      const typed = await page.evaluate(() => {
        ...
        if (!el) return false;
        el.focus();
        el.value = 'we need to revisit this';
  ...
  },
  {
    ...
    async assert(page) {
      return await page.evaluate(() => {
        ...
        if (!m) return [];
        const held =
          typeof room === 'function' && room() && room().objects ? room().objects.length : null;
        if (held == null) return [];
        ...
        if (/outside your list|beyond your list|not in your list|everything else/i.test(t))
          return [];
        return [
          'the pin trailer says "' +
            m[0] +
            '" over ' +
            m[2] +
            ' records while the room holds ' +
            held +
            ', so the number is scoped to the records outside the pin and the sentence does not say so — ' +
            'the other half of the same line is scoped to the room and does: ' +
            JSON.stringify(t),
        ];
      });
    },
        ...
        return el.value === 'the July AWS bill'
          ...
          : [
              'the evidence a person typed was destroyed by an unrelated re-render — the box now holds ' +
                ...
                ", and this file's own rule is that \"deleting somebody's draft to be " +
  ...
  },
  {
    ...
    async run(page) {
      ...
      const typed = await page.evaluate(() => {
        ...
        if (!el) return false;
        el.focus();
        el.value = 'the flag was flipped back';
        el.dispatchEvent(new Event('input', { bubbles: true }));
        return true;
      });
      if (!typed) return 'the reopen prompt has no reason field';
        ...
        return el.value === 'the flag was flipped back'
          ...
          : [
              'the reason a person typed was destroyed by an unrelated re-render — the box now holds ' +
                JSON.stringify(el.value) +
                ', under a hint promising it is kept word for word',
            ];
      });
    },
  },
  {
    ...
    async run(page) {
      const typed = await page.evaluate(() => {
        ...
        if (!el) return false;
        el.focus();
        el.value = 'half a message about the cutover';
        el.dispatchEvent(new Event('input', { bubbles: true }));
        return true;
      });
    ...
    async assert(page) {
      return await page.evaluate(async () => {
        ...
        if (el.value !== 'half a message about the cutover')
          out.push(
            'answering a decision from the pin destroyed a draft it never read — the composer now holds ' +
              JSON.stringify(el.value),
          );
        ...
        box.focus();
        box.value = 'we hold until the freeze clears';
        ...
        if (after && after.value !== '')
          out.push(
            'a typed answer left its own characters in the composer after recording them as the ' +
              'statement of record — the box still holds ' +
  ...
  },
  {
    ...
    async run(page) {
      const seen = [];
      const sample = async () => {
        seen.push(...(await page.evaluate(ECHO_SCAN)));
      };
      ...
      await page.fill(
        '#cinput',
        'Dual-write runs to the freeze and the legacy tail is force-re-authed on 15 Aug',
      );
      ...
      if (!(await clickText(page, 'Answer — retention is 90 days')))
        return "Q1's answer not offered";
      await sample();
      ...
      if (!(await clickText(page, 'Keep dual-write through 14 Aug')))
        return 'the reopened decision offers no option';
      await sample();
      await page.evaluate(
        (f) => {
          window.__r18d1 = f;
  ...
  },
  {
    ...
    async run(page) {
      if (!(await clickText(page, 'Keep dual-write through 14 Aug')))
        return "P1's answer not offered";
    ...
    async assert(page) {
      return await page.evaluate(() => {
        ...
        return count
          ? [
              'the pin announces the same emptiness twice: the count slot says ' +
                JSON.stringify(count) +
                ' over an empty state that already says ' +
                JSON.stringify(String(box.textContent || '').trim()),
            ]
          : [];
      });
    },
  ...
  },
  {
    ...
    async run(page) {
      ...
      if (!(await clickIn(page, '#lensBody .oitem[data-lens-obj="D2"]')))
        return "D2 is not in this room's lens";
      if (!(await clickIn(page, '#rcReopen'))) return 'the receipt offers no Reopen';
      if (!(await clickIn(page, '#rgo'))) return 'the reopen prompt offers no confirm';
      const before = await page.evaluate(() => {
        ...
        return {
          ...
          room: S.roomId,
          declared: b.dataset.jumproom || null,
    ...
    async assert(page) {
      return await page.evaluate(() => {
        ...
        if (b.declared && b.label.indexOf('#' + b.declared) < 0)
          out.push(
            'a route that goes to #' + b.declared + ' does not name it: ' + JSON.stringify(b.label),
          );
        if (/the row that carried it re-renders to say so/i.test(b.note) && b.declared)
          out.push(
            'the receipt for ' +
              b.obj +
              ' says the row that carried it re-renders to say so — that row is in #' +
              b.declared +
              ' and does not render here at all',
          );
        return out;
      });
    },
  },
  {
    ...
    async run(page) {
      if (!(await clickIn(page, '#railRooms .rrow[data-room="identity-service"]')))
        return 'no route to #identity-service';
        ...
        document.querySelectorAll('.corr .bd button[data-jump]').forEach((b) => {
          ...
          if (/still in the room/i.test(label))
            out.push(
              'a route into #' +
                room +
                ' says the thing is still in the room you are standing in: ' +
        ...
        document.querySelectorAll('#pinList [data-obj]').forEach((card) => {
          ...
          out.push(
            'the WHY line on ' +
              ...
              ' renders ' +
              where +
              ' it in the pin, which is where ' +
              'this sentence is painted. On screen: ' +
              (found.length
                ? found
                    .map((f) => f.id + ' at y=' + f.y + ' in #' + f.host + ' (' + f.side + ')')
                    .join(', ')
    ...
    async assert(page) {
      return await page.evaluate(() => {
        ...
        document.querySelectorAll('body *').forEach((el) => {
          ...
          out.push(
            'a visible element paints its own text into an ellipsis: scrollWidth ' +
              el.scrollWidth +
              ' / clientWidth ' +
              el.clientWidth +
              ' on ' +
              (el.id ? '#' + el.id : el.className || el.tagName) +
              ' — ' +
        ...
        if (!(hit === j || j.contains(hit)))
          out.push(
            'the point at the centre of ' +
              JSON.stringify(label) +
              ' hit-tests to something else — the click ' +
              'a reader aims at this control lands on ' +
              JSON.stringify(((hit && hit.outerHTML) || '').slice(0, 60)),
          );
        (window.__occl().findings || []).forEach((f) => {
          if (f.text.indexOf('go to it') < 0) return;
          out.push(
            "a control's own label is painted over by another control: " +
              JSON.stringify(label) +
              ' loses ' +
              f.n +
              ' character(s) / ' +
  ...
  },
  {
    ...
    async assert(page) {
      const msgs = await page.evaluate(() =>
        Array.from(document.querySelectorAll('#feed .mrow'))
          .map((el) => el.dataset.msg)
          .filter(Boolean),
      );
      const out = [];
      ...
      for (const m of msgs) {
        ...
        const f = await page.evaluate(
          (m) => window.__occl().findings.filter((x) => x.where === m),
          m,
        );
        if (f.length) rows++;
        f.forEach((x) => {
          out.push(
            'hovering a row covers ' +
              x.n +
              ' of its own characters (' +
              x.px +
              'px) — ' +
              JSON.stringify(x.covered) +
              ' out of ' +
      ...
      if (out.length)
        out.unshift(
          rows +
            ' of ' +
            msgs.length +
            ' rows lose characters when hovered at ' +
      ...
      return await page.evaluate(() => {
        ...
        if (typeof window.__numberReach === 'function') {
          window
            ...
            .forEach((l) => {
              out.push('a number in a minted sentence is read by nothing: ' + l);
            });
          return Array.from(new Set(out));
        }
        ...
        out.push(
          'this build cannot say how any number in a minted sentence was read — window.__numberReach is missing',
        );
        document.querySelectorAll('[data-said], [data-said-parts]').forEach((el) => {
          const t = (el.textContent || '').replace(/\s+/g, ' ').trim();
          if (!/^[\d\s/·,.-]*\d[\d\s/·,.-]*$/.test(t)) return;
          const box = (window.PAINTED || [])[
            Number(el.dataset.said != null ? el.dataset.said : el.dataset.saidParts)
          ];
          const owners = box ? new Set((box.claims || []).map((c) => c.of)) : null;
          out.push(
            "a minted sentence's entire text is a numeral, so no phrasing pattern can reach it: " +
              JSON.stringify(t) +
              (owners ? ' (minted from ' + (box.claims || []).length + ' reads)' : ''),
          );
        });
        return Array.from(new Set(out));
      });
  ...
  },
  {
    ...
    async run(page) {
      ...
      if (!(await clickText(page, 'Answer — retention is 90 days')))
        return 'the one-click answer is not offered';
      return null;
    },
    async assert(page) {
      return await page.evaluate(() => {
        ...
        document.querySelectorAll('.rrow .nm, .rrow .ct, .rrow .pill').forEach((el) => {
          ...
          if (clips && el.scrollWidth > el.clientWidth + 1)
            out.push(
              'a rail row is painting a room name into an ellipsis at ' +
                window.innerWidth +
                'px: ' +
      ...
      return await page.evaluate(() => {
        ...
        const w = document.createTreeWalker(document.body, NodeFilter.SHOW_TEXT, {
          acceptNode(n) {
            ...
            return /\d/.test(String(n.nodeValue))
              ? NodeFilter.FILTER_ACCEPT
              : NodeFilter.FILTER_REJECT;
          },
        });
        ...
        for (n = w.nextNode(); n; n = w.nextNode()) {
          ...
          out.push(
            'a count of records sits inside no mint: ' +
              ((host && host.id) || n.parentElement.className) +
              ' · ' +
    ...
    async assert(page) {
      return await page.evaluate(() => {
        ...
        return el.scrollWidth > el.clientWidth + 1 || el.scrollHeight > el.clientHeight + 1
          ? [
              "the room's topic is truncated at " +
                window.innerWidth +
                'px: ' +
                el.scrollWidth +
                ' > ' +
      ...
      return await page.evaluate(() => {
        ...
        document.querySelectorAll('body *').forEach((el) => {
          ...
          if (/['’]s$/i.test(last) || TAIL.indexOf(last.toLowerCase()) >= 0)
            out.push(
              'a rendered line is unfinished: ' +
                JSON.stringify(raw.length > 100 ? '…' + raw.slice(-95) : raw),
            );
        });
        return Array.from(new Set(out));
      });
      ...
      return await page.evaluate(() => {
        ...
        document.querySelectorAll('.syl .when, .syl').forEach((el) => {
          ...
          if (/count(s)? rows|counts the items behind|this counts the rows/i.test(t))
            out.push(
              'a divider reconciles two counts in prose instead of naming their units: ' +
  ...
  },
  {
    ...
    async assert(page) {
      return await page.evaluate(() => {
        const out = [];
        [
          ['lensSummary', 'the lens'],
          ['pinTrailer', 'the pin trailer'],
        ].forEach(([id, who]) => {
          ...
          if (m[2] == null)
            out.push(
              who +
                ' prints "' +
                m[0] +
                '" with no denominator, and another surface on the same screen ' +
                'counts the same word over a different set: ' +
      ...
      return await page.evaluate(() => {
        ...
        document
          ...
          .forEach((el) => {
            ...
            for (let p = el; p && p.nodeType === 1; p = p.parentElement) {
              if (
                typeof p.getAnimations === 'function' &&
                p.getAnimations().some((a) => a.playState === 'running')
              )
                return;
            ...
            if (h === el || (h && el.contains(h)))
              out.push(
                'a control at zero opacity still takes the pointer: ' +
  ...
  },
  {
    ...
    async run(page) {
      ...
      if (!(await clickText(page, 'Does legal approve')))
        return 'Q1 not reachable after the hand-off';
    ...
    async assert(page) {
      return await page.evaluate(() => {
        ...
        if (hist.length < 3)
          out.push('the round trip did not land: assignee history is ' + JSON.stringify(hist));
        ...
        const bad = [];
        document
          .querySelectorAll('[data-obj="Q1"] button, [data-lens-obj="Q1"] button')
          .forEach((b) => {
            if (
              b.querySelector('[data-said], [data-said-parts]') ||
              b.closest('[data-said], [data-said-parts]')
            )
              return;
            ...
            hist.forEach((v) => {
              ...
              bad.push(
                'a control inside Q1 says ' +
                  JSON.stringify(v) +
                  ' — a value the assignee history holds — ' +
                  'without reading it and without declaring that it writes it: ' +
  ...
  },
  {
    ...
    async run(page) {
      ...
      if (!(await clickText(page, 'Answer — retention is 90 days')))
        return 'the answer is not offered again';
      return null;
    },
  },
  {
      ...
      return await page.evaluate(() => {
        ...
        if (owed === 0 && Number(a[1]) !== Number(b[1]))
          out.push(
            'the lens says "' +
              a[0] +
              '" and the pin trailer says "' +
              b[0] +
              '" over the same ' +
              b[2] +
              ' records, with nothing owed, so there is no scope difference to explain it',
          );
        return out;
      });
    },
          ...
          if (!/overlap|also sit|of the \d+|of \d+ objects/i.test(said))
            out.push(
              '"' +
                ...
                '" has ' +
                overlap +
                ' object(s) that also sit under another objective and ' +
                'nothing on it — text or accessible name — says the counts overlap: ' +
                JSON.stringify(said.trim().slice(0, 90)),
            );
        });
        return out;
      });
      ...
      return await page.evaluate(() => {
        ...
        Object.keys(named).forEach((id) => {
          ...
          if (!el.closest('[data-said], [data-said-parts], [data-obj], [data-agg]'))
            out.push(
              named[id] +
                ' counts records and sits inside no mint and no declared subject: ' +
                JSON.stringify((el.textContent || '').trim()),
            );
        });
        ...
        document.querySelectorAll('#railRooms .pill').forEach((p) => {
          ...
          for (n = w.nextNode(); n; n = w.nextNode()) {
            ...
            if (n.parentElement.closest('[data-said], [data-said-parts], [data-obj], [data-agg]'))
              continue;
            out.push(
              'a rail owed badge counts records and its digits sit inside no mint and no declared subject: ' +
  ...
  },
  {
    ...
    async run(page) {
      if (!(await clickText(page, 'Dual-write stays on until parity')))
        return "D1's lens item not found";
      return null;
    },
    async assert(page) {
      return await page.evaluate(() => {
        const out = [];
        if (typeof window.__frictionTiers !== 'function')
          return ['the page reports no friction tiers'];
        const t = window.__frictionTiers();
        const open = S.ui.receiptId;
        (t.verified || []).forEach((k) => {
          if (k.indexOf('|') < 0) {
            out.push(
              'a friction tier is credited as "' +
                k +
                '" with no object — one control anywhere ' +
          ...
          if (tier === '2' && who !== open)
            out.push(
              'tier 2 is credited for ' +
                who +
                ' while the open receipt is ' +
                open +
  ...
  },
  {
    ...
    async run(page) {
      ...
      await page.evaluate(() => {
        window.__predicted = null;
      });
      await page.evaluate((b) => {
        window.__predicted = b;
      }, before);
      if (!(await clickText(page, 'Dual-write stays on until parity')))
        return "D1's lens item not found";
      if (!(await clickText(page, 'Reopen'))) return 'the receipt offers no Reopen';
    ...
    async assert(page) {
      return await page.evaluate(() => {
        ...
        return got === want
          ? []
          : [
              'offeredAfter(D1,"reopen") predicted [' +
                want +
                '] and the reopen actually offers [' +
                got +
      ...
      return await page.evaluate(() => {
        ...
        if (typeof clausesOf === 'function') {
          ...
          if (!cl.some((c) => /answer/.test(c) && /kept/.test(c)))
            out.push(
              "clausesOf() puts a field's own word and the quantifier about it in different clauses, so the " +
                'pairing rule cannot see them together: ' +
                JSON.stringify(cl),
            );
        }
      ...
      return await page.evaluate(() => {
        ...
        document.querySelectorAll('.syl .cnt').forEach((b) => {
          ...
          if (/^0\b/.test(t) || b.disabled)
            out.push(
              'a divider paints a chip that filters nothing: ' +
                JSON.stringify(t) +
                (b.disabled ? ' [disabled]' : ''),
            );
        });
        document.querySelectorAll('.syl .when').forEach((w) => {
          ...
          if (/\b0 unseen\b/.test(t) && /never counted back to you/.test(t))
            out.push(
              'a divider prints a zero and then two clauses explaining the zero: ' +
                JSON.stringify(t.slice(0, 120)),
            );
        });
        return out;
    ...
    async assert(page) {
      return await page.evaluate(() => {
        ...
        document.querySelectorAll('#feed .tag').forEach((el) => {
          ...
          Object.keys(o.recorded || {}).forEach((f) => {
            (o.recorded[f] || []).forEach((e) => {
              ...
              if (
                ...
                out.push(
                  'an unminted row tag states a value ' +
                    o.id +
                    "'s record holds for " +
                    f +
                    ': ' +
                    JSON.stringify(el.textContent.trim().slice(0, 50)) +
                    ' vs ' +
                    e.value,
                );
            });
  ...
  },
  {
    ...
    async assert(page) {
      return await page.evaluate(() => {
        ...
        const out = [];
        if (q.owedTo !== S.me)
          out.push('taking it back did not put it back on your list: owedTo=' + q.owedTo);
        if ((q.recorded.assignee || []).length < 3)
          out.push(
            'the hand-off and the take-back are not both on the record: ' +
    ...
    async assert(page) {
      return await page.evaluate(() => {
        ...
        document.querySelectorAll('[data-quoted^="msg:"]').forEach((el) => {
          ...
          if (shown.length > 1 && /^[“"]/.test(shown) && /[”"]$/.test(shown))
            shown = shown.slice(1, -1).trim();
          ...
          const midWord =
            full.indexOf(shown) === 0 &&
            /[\w'’-]/.test(nextChar) &&
            /[\w'’-]/.test(shown.charAt(shown.length - 1));
          if (midWord)
            out.push(
              "a quotation stops mid-word and is presented as the person's words: …" +
                JSON.stringify(shown.slice(-34)) +
                ' — the record continues ' +
                JSON.stringify(nextChar + full.slice(shown.length + 1, shown.length + 12)),
            );
          else if (!declared && !hadEllipsis)
            out.push(
              'a quotation is shortened before it is painted, with nothing on it saying so: ' +
                shown.length +
                ' of ' +
                full.length +
                ' characters, …' +
  ...
  },
  {
    ...
    async assert(page) {
      return await page.evaluate(() => {
        ...
        const claimsLanding = /(proposals?|anything derived)\s+lands?\s+in\s+current\s+state/i.test(
          st.claim || '',
        );
        if (!grew && (claimsInterpretation || claimsLanding))
          out.push(
            'the composer states a behaviour the artifact does not have: ' +
              JSON.stringify(st.claim) +
              ' — a proposal-shaped message was sent and the record went from ' +
              st.before +
              ' objects to ' +
              after,
          );
        ...
        document.querySelectorAll('#cstate').forEach((el) => {
          if (/interpretation\s+is\s+skipped/i.test(el.textContent || ''))
            out.push(
              'the bound composer says interpretation is SKIPPED, which tells the reader it otherwise runs: ' +
                JSON.stringify((el.textContent || '').trim()),
            );
        });
        return out;
  ...
  },
  {
    ...
    async run(page) {
      if (!(await clickText(page, 'Keep dual-write through 14 Aug')))
        return "P1's answer not offered";
      if (!(await clickText(page, 'the receipt for P1')))
        return 'the transition row offers no receipt';
      if (!(await clickText(page, 'Reopen'))) return 'the receipt offers no Reopen';
      await page.evaluate(() => {
        const b = document.querySelector('[data-verify]');
        if (b) b.click();
      });
    ...
    async assert(page) {
      return await page.evaluate(() => {
        ...
        document.querySelectorAll('input[placeholder], textarea[placeholder]').forEach((el) => {
          ...
          out.push(
            "a prompt's own question is clipped at " +
              window.innerWidth +
              'px: #' +
              el.id +
              ' needs ' +
        ...
        if (box) {
          ...
          if (!/your own sentence|word for word|verbatim|attributed to you/i.test(t))
            out.push(
              'the reopen prompt never says on screen that a typed reason is kept as your own words: ' +
                JSON.stringify(t.slice(0, 120)),
            );
          if (/nothing is ever written in yours/i.test(t))
            out.push(
              "the reopen prompt's only readable sentence contradicts what the write does — " +
                'a typed reason IS rendered attributed, permanently: ' +
                JSON.stringify(t.slice(0, 120)),
            );
        }
        return Array.from(new Set(out));
...
const THEMES = String(argOf('--themes', 'light,dark'))
  .split(',')
  .map((t) => t.trim())
  .filter(Boolean);
  ...
  for (const rp of REPROS) {
    ...
    for (const width of widths)
      for (const theme of THEMES) {
        ...
        inv.forEach((t) => {
          merged.errors.push(at + '  ' + t);
        });
        asserts.forEach((t) => {
          merged.asserts.push(at + '  ' + t);
        });
        if (inv.length + asserts.length && merged.firedAt.indexOf(at) < 0) merged.firedAt.push(at);
      }
    merged.classes = Array.from(
      new Set(merged.errors.map((t) => classOf(t.replace(/^@[\d]+\/\w+\s+/, '')))),
    );
    rows.push(merged);
    const bad = merged.errors.length + merged.asserts.length;
    log(
      `  ${bad ? 'FIRES' : 'clean'}  ${rp.id} — ${merged.errors.length} console error(s), ${merged.classes.length} class(es), ` +
        `${merged.asserts.length} assertion(s) over ${widths.length} width(s) × ${THEMES.length} theme(s)` +
        (bad
          ? ` [fires at ${merged.firedAt.join(', ')} of ${widths.length * THEMES.length} width/theme pairs]`
          : '') +
        (merged.skip ? ` [path incomplete: ${merged.skip}]` : ''),
    );
    merged.errors.slice(0, 6).forEach((t) => {
      log(`      ${t.slice(0, 210)}`);
    });
    ...
    merged.asserts.slice(0, 80).forEach((t) => {
      log(`      ! ${t}`);
    });
  }
  return rows;
...
async function passOcclusion(browser, log) {
  ...
  log(
    `  ${samples} samples over ${hovers} pointer positions, at ${WIDTHS.join(', ')} in light and dark`,
  );
  const list = Array.from(seen.values());
  if (!list.length) log(`  nothing visible is painted over by anything else you can see`);
  list.forEach((f) => {
    log(
      `  PAINTED OVER @${f.w} ${f.theme}: ${f.n} character(s) / ${f.px}px of ${f.owner} (${f.where}) — ` +
...
if (!SKIP_WALK) {
  const cls = new Map();
  sq.violations.forEach((v) => {
    const c = classOf(v.text);
    if (!cls.has(c)) cls.set(c, v);
  });
  cls.forEach((v, c) => {
    log(`     ${c}`);
    log(`        path: ${v.path.join('  →  ') || '(on load)'}`);
  });
}
if (!SKIP_WALK) {
  const notDriven = Array.from(sq.offeredSteps)
    .filter((s) => !sq.drivenSteps.has(s))
    .sort();
  log(
    `  (object, action, driver) steps offered: ${sq.offeredSteps.size} · driven: ${sq.drivenSteps.size}`,
  );
  /* a starved load is an environment fact and it is reported, not swallowed */
  if (SEQ_RELOADS)
    log(
      `  state loads that timed out and were retried: ${SEQ_RELOADS} (bounded at one retry each)`,
    );
  if (notDriven.length) {
    log(
      `  OFFERED BUT NOT DRIVEN — an action the page offers with no control the harness could reach:`,
    );
    notDriven.forEach((s) => {
      log(`     - ${s}   [${sq.unreachable.get(s) || 'no reason recorded'}]`);
    });
  }
}
...
if (!SKIP_WALK) {
  ...
  if (UNCOVERED_OUT) {
    fs.writeFileSync(UNCOVERED_OUT, body);
  ...
  if (committed == null) {
    ...
    if (live !== have) {
      ...
      const liveSet = new Set(uncoveredLines),
        haveSet = new Set(have.split('\n').filter(Boolean));
      uncoveredLines
        .filter((l) => !haveSet.has(l))
        .forEach((l) => {
          log(`     + live only: ${l}`);
        });
      Array.from(haveSet)
        .filter((l) => !liveSet.has(l))
        .forEach((l) => {
          log(`     - file only: ${l}`);
        });
  ...
  const block = (src.split('AND WHAT THE WALK DOES NOT COVER')[1] || '').split(
    'function uncoveredRecordWords',
  )[0];
  const quoted = Array.from(block.matchAll(/`([^`]{4,})`/g))
    .map((m) => m[1])
  ...
  if (bogus.length) {
    ...
    docFails.push(
      `the uncovered-set comment names ${bogus.length} example(s) the walk cannot produce`,
    );
  } else if (quoted.length) {
    log(
      `  the uncovered-set comment names ${quoted.length} example(s), all present in the live walk`,
    );
  } else {
...
if (!SKIP_WALK) {
  ...
  const unread = numberLines.filter((l) =>
    /^mint\s+(UNREAD|UNDECLARED|DECLARED-NOTHING|UNMEASURED|MISMEASURED)/.test(l),
  );
  const header =
  ...
  if (COUNTS_OUT) {
    fs.writeFileSync(COUNTS_OUT, body);
  ...
  if (committed == null) {
    ...
    if (live !== have) {
      ...
      const liveSet = new Set(numberLines),
        haveSet = new Set(have.split('\n').filter(Boolean));
      numberLines
        .filter((l) => !haveSet.has(l))
        .forEach((l) => {
...
const en = SKIP_WALK
  ? { seen: new Map(), violations: [], sessions: 0 }
  : await passEnumerate(browser, log);
...
if (!SKIP_WALK) {
  ...
  notDriven.forEach(([k, v]) => {
    log(`     - ${k}   [${v.reason || 'reachable but never selected'}]`);
  });
  log(`  invariant violations during enumeration: ${en.violations.length}`);
  const cls = new Map();
  en.violations.forEach((v) => {
    const c = classOf(v.text);
    if (!cls.has(c)) cls.set(c, v);
  });
  cls.forEach((v, c) => {
    log(`     ${c}`);
    log(`        path: ${v.path.slice(-4).join('  →  ')}`);
  });
}
...
const totalClasses = new Set(
  sq.violations
    .concat(en.violations, rn.violations)
    .map((v) => classOf(v.text))
...
sq.ctlSeen.forEach((v, k) => {
  inventory.set(k, { visible: v.visible, driven: false });
});
all.forEach(([k, v]) => {
...
if (SKIP_WALK)
  log(
    `  --only ${ONLY}: no controls enumerated, no states walked, no artifact compared. This run reports ` +
...
if (!SKIP_WALK) {
  ...
  log(
    `  states reached at depth ${sq.depth}:${' '.repeat(Math.max(1, 29 - String(sq.depth).length))}${statesReached}`,
  );
  ...
  log(
    `  viewports: enumeration at ${SEQ_WIDTH}x${HEIGHT} · enumeration/random/repro passes at ${WIDTHS.join(', ')} (height ${HEIGHT})`,
  );
  sq.levels.forEach((l) => {
    log(
      `     depth ${l.depth}: ${l.states} new state(s) from ${l.transitions} transition(s)` +
        (l.truncated ? ` · ${l.truncated} state(s) LEFT UNEXPANDED (budget ${sq.budget})` : '') +
        (l.frontier
          ? ` · ranking: ${l.inertCtl}/${l.frontier} tied at zero control novelty, ${l.inertBoth} still tied after step novelty`
          : ''),
    );
  });
  {
    ...
    const cut = sq.levels.filter(
      (l) => l.truncated && l.frontier && l.frontier - l.inertBoth <= l.expanded,
    );
    const partial = sq.levels.filter(
      (l) => l.truncated && l.frontier && l.inertBoth && l.frontier - l.inertBoth > l.expanded,
    );
    if (cut.length)
      cut.forEach((l) => {
        log(
          `  THE FRONTIER RANKING DID NOT CHOOSE AT DEPTH ${l.depth} — ${l.inertBoth} of ${l.frontier} ` +
    ...
    if (partial.length)
      partial.forEach((l) => {
        log(
          `  the frontier ranking chose the expanded set at depth ${l.depth} (${l.inertBoth} of ` +
  ...
  if (truncatedLevels.length)
    log(
      `  BREADTH IS NOT COMPLETE at depth ${truncatedLevels.map((l) => l.depth).join(', ')} — ` +
        `${truncatedLevels.reduce((n, l) => n + l.truncated, 0)} state(s) were never expanded. ` +
        `This is a sampled level, not a covered one.`,
    );
  else log(`  breadth is complete at every depth up to ${sq.depth}`);
  {
    const undriven = Array.from(inventory.entries()).filter(([, v]) => !v.driven);
    log(`  THE UNEXERCISED SET (${undriven.length}):`);
    undriven.forEach(([k, v]) => {
      log(`     - ${k}   [${v.visible ? 'reachable, never selected' : 'never on screen'}]`);
    });
  }
}
if (docFails.length)
  log(`  DOCUMENTATION CHECKS FAILED (${docFails.length}): ${docFails.join(' · ')}`);
log(
  `  violations — sequence ${sq.violations.length} · enumeration ${en.violations.length} · ` +
    ...
    JSON.stringify(
      {
        file: FILE,
        controls: {
          enumerated: inventory.size,
          driven: drivenKeys,
          unexercised: Array.from(inventory.entries())
            .filter(([, v]) => !v.driven)
            .map(([k, v]) => ({ key: k, visible: v.visible })),
        },
        sequences: {
          depth: sq.depth,
          budget: sq.budget,
          ...
          stepsNotDriven: Array.from(sq.offeredSteps)
            .filter((s) => !sq.drivenSteps.has(s))
            .sort(),
        },
        sequenceViolations: sq.violations,
        enumeration: en.violations,
        random: rn.violations,
        repros: rp,
