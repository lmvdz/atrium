...
- **The checker's surface list has to be derived, not remembered — and written down where the next reader can audit it.** A hand-maintained list of selectors is the same denylist one layer up. The list is taken from the object's own shape, every property the page paints words from, with the reason each excluded one is already covered by a named invariant. Round 10's first attempt at that list was still incomplete, and two blind foreign-lineage reviewers independently found the same two gaps in it: the correction entry's own prose `fact` line, sitting beneath a header and two operands that were all record reads, and the receipt's state line, the loudest sentence in a receipt, composed off the object rather than out of its history. The list being written down is what made it falsifiable; the reason it was wrong is that it was written from memory of the code rather than from the shape of the data.
