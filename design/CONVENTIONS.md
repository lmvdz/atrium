...
- **The checker's surface list has to be derived, not remembered.** A hand-maintained list of selectors is the same denylist one layer up. The list is taken from the object's own shape — every property the page paints words from — and stated in the source, with the reason each of the remaining properties (the statement itself, the provenance) is already covered by a named invariant.
