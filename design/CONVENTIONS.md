...
- **The scope of that rule, stated 2026-08-05, because it was read wider than it holds.** It was written when every token was verified byte-identical to the v6 corpus: with provenance like that, a failing pair is always a usage error, because the value is evidence. **WIRE replaced the values**, and WIRE's own `--tx2` (`#77827b`) measured **4.49:1 on `--bg3`/`--bg5`/`--bg7` and 4.19:1 on `--bg6`** — under AA on every surface darker than `--bg2`, which is where most of the component library's secondary text sits (`.actor`, `.time`, `.tag`, `.glyph`, `.link`, `.systemBody`). No choice of token fixes that: `--tx1` flattens the ramp the palette exists to express, and `--tx3` is worse. It was corrected in the **value** — `--tx2` is now `#7e8982`, which clears 4.5 on all eight surfaces (4.60 at worst, on `--bg6`). The rule stands where its premise does: **when a token's provenance is a verified source, fix the usage; when the palette itself is the thing that failed, fix the palette and say so here.**
- **`--tx3` and `--tx4` cannot carry text at AA, and never could.** `--tx3` measures 2.13–2.43:1 across WIRE surfaces and 2.59–2.87:1 on the pre-v8 palette; `--tx4` measures 1.74–1.99:1 and 1.78–1.98:1. Neither has ever reached 4.5 against anything, in either palette. They are tokens for **rules, borders and glyph fills**, not for words. The ramp description above calls `--tx4` "the quietest legible thing on the page" — read that as quietest *visible*, because legible it is not. The v8 batch used both for real text (tree column headers, per-row lens/spend/age cells, the fold pane) and every one of those was a contrast failure at every width in both themes.
