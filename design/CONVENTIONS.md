...
So a row carries a **checksum** of the record it was minted from, and the render boundary recomputes it from the record it is about to resolve against. That is not the carried-field pattern this round deleted, and the distinction is worth stating precisely: **an attribution is a claim about who, and a checksum is a claim that two registers are the same register.** Nothing about it is printed, nothing about it is read for its value, and a mismatch throws rather than picking a winner. Every field a reader can see is in it, because the register that disagrees about the field you left out is the one that gets through.
