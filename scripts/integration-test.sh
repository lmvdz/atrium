...
# the port already is. It was hardcoded, so two agents running this concurrently
# on different ports still shared one compose project — the second `up` adopted
# the first's container and the trap on either one's exit tore the other's
# database out from under it, mid-suite. A port without a project is half an
# isolation.
